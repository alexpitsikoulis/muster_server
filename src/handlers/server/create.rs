...
pub async fn create(
    body: web::Json<CreateServerRequestData>,
    ...
    match req.headers().get("Authorization") {
        Some(header) => {
            let token = match header.to_str() {
                ...
                Err(e) => {
                    tracing::error!(
                        "500 - Authorization header could not be cast to string: {:?}",
                        e
                    );
                    return HttpResponse::BadRequest().body(format!(
                        "Authorization header could not be cast to string: {:?}",
                        e
                    ));
                }
            };
            match get_claims_from_token(token) {
                Ok(claims) => {
                    ...
                    if body.name.len() > 50 {
                        tracing::error!(
                            "400 - Server name is too long, must be no more than 50 characters"
                        );
                        return HttpResponse::BadRequest()
                            .body("Server name is too long, must be no more than 50 characters");
                    }
                    let server: Server = CreateServerRequestDataWithOwner {
                        ...
                        owner_id: match Uuid::parse_str(&subject) {
                            ...
                            Err(_) => {
                                tracing::error!(
                                    "400 -  Malformed JWT provided, sub is not a valid UUID"
                                );
                                return HttpResponse::BadRequest()
                                    .body("Malformed JWT provided, sub is not a valid UUID");
                            }
                        },
                    }
                    .into();
